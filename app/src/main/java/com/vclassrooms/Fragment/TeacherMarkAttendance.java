 ...
 */
public class TeacherMarkAttendance extends Fragment implements DatePickerDialog.OnDateSetListener {
    View mview;
    AppUtils appUtils;
    ...
    public void MarkAttendancedata()
    {
        ...
        for(int i=0;i<studentlist.size();i++){
            ...
            markAttendanceEnum.setRole_Id(constatnts.TeacherRole);
            markAttendanceEnum.setStandard_id(0) ;
            markAttendanceEnum.setIntDivision_id(0);
    ...
    private void MarkAttendanceApi(List<MarkAttendanceEnum> markListStudents) {
        try {
            appUtils.showProgressbar(context);
            Call<CommonSuccessResponse> call = ApiService.buildService(context).MarkAttendanceDetail(strAuth,"InsertEmployee",markListStudents);
    ...
    private void init() {
        ...
        appUtils.setText(txt_standard,"Teacher");
    ...
    private void getMarkAttendanceStudentListApi(String strDateSelected) {
        try {
            ...
            appUtils.showProgressbar(context);
            Call<StudentAttendanceDetailResponse> call = ApiService.buildService(context).getStudentAttendanceDatewise(strAuth,"getEmployeeAttendance",constatnts.TeacherRole, strDateSelected,strSchoolId,strAcademicId,"0","0",strUserId);
    ...
    private void getTeacherListApi() {
        try {
            appUtils.showProgressbar(context);
                        ...
                        if (response.body().getStatusCode().equals(0)) {
                            if(response.body().getData().getDirectoryeDetail()!=null && response.body().getData().getDirectoryeDetail().size()>0){
                                ...
                                for(int i=0;i<response.body().getData().getDirectoryeDetail().size();i++){
                                    ...
                                    attendanceDetail.setUserId(response.body().getData().getDirectoryeDetail().get(i).getEmpId());
                                    attendanceDetail.setDivisionId(Integer.valueOf(0));
                                    attendanceDetail.setStandardId(Integer.valueOf(0));
                                    attendanceDetail.setImageURL(response.body().getData().getDirectoryeDetail().get(i).getImageURL());
                                    attendanceDetail.setName(response.body().getData().getDirectoryeDetail().get(i).getFirstName());
